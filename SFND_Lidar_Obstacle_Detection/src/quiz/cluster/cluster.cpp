...
void proximity_checker(std::vector<std::vector<float>> points, int id, KdTree* tree, std::vector<int>& processed_ids,
                       std::vector<int>& sub_cluster, float distanceTol)
{
    ...
    {
        if (!std::count(processed_ids.begin(), processed_ids.end(), *nearby_it))
        {
            proximity_checker(points, *nearby_it, tree, processed_ids, sub_cluster, distanceTol);
        }
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<int> processed_ids;  //speed improvement -- make processed_ids vector of bool instead of int

	//int id = 0;
	for(int i=0; i<points.size(); ++i)
    {
        //std::vector<float> curr_point = points[i];

	    if (std::count(processed_ids.begin(), processed_ids.end(), i))
        {
	        //id++;
	        continue;
        }
        std::vector<int>sub_cluster;
        proximity_checker(points, i, tree, processed_ids, sub_cluster, distanceTol);
