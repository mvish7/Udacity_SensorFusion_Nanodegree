...
int main(int argc, const char *argv[])
{
    ...
    result <<"imageIndex" << ","  << "detectorType" << ","<< "descriptorType" << "," << "totalTime"<<","<< "TTC" <<","<<endl;
    ...
    for (it1 = detector_names.begin(); it1!= detector_names.end(); ++it1) {
        ...
        for (it2 = descriptor_names.begin(); it2 != descriptor_names.end(); ++it2)
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth) {
                ...
                bVis = false;
                if (bVis) {
                    std::string id = imgNumber.str();
                    show3DObjects((dataBuffer.end() - 1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), id,
                                  true);
                }
                bVis = false;
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    matchDescriptors((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints,
                                     ...
                                     matches, descriptorType, matcherType, selectorType, num_matches);
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin();
                            ...
                            prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                        {
                            ...
                            if (bVis) {
                                ...
                                string img_name = "f_" + imgNumber.str() + ".png";
                                string img_filename = img_path.append(img_name);
                                cv::imwrite(img_filename, visImg);*/
                    ...
                    result <<imgIndex <<"," << detectorType << ","<< descriptorType << ","<< t_detector + t_descriptor<<","<<ttcCamera<<","<< endl;
                }
            } // eof loop over all images
        }  //eof loop  of descriptors
    } //eof loop of detector
